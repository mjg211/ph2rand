single_double_des_one_stage        <- function(alpha, beta, delta, ratio,
                                               pi0_null, pi0_alt, n0max,
                                               summary) {
  params               <- search_parameters(1, "single_double", n0max, ratio)
  feasible             <-
    single_double_des_one_stage_cpp(alpha, beta, delta, params$poss_n0,
                                    params$poss_n1, params$poss_x,
                                    params$poss_y, (length(pi0_null) == 1),
                                    pi0_null, (length(pi0_alt) == 1), pi0_alt,
                                    summary)
  if (feasible[1, 1] > 0) {
    if (summary) {
      message(uc("two_elip"), "feasible designs identified in the range of ",
              "considered sample sizes. Identifying the optimal design",
              uc("two_elip"))
    }
    nrow_feasible      <- nrow(feasible)
    if (nrow_feasible == 1) {
      feasible         <- matrix(c(1, feasible[, 1],
                                   params$poss_n1[feasible[, 1]],
                                   feasible[, -1]), 1)
    } else {
      feasible         <- cbind(1:nrow(feasible), feasible[, 1],
                                params$poss_n1[feasible[, 1]], feasible[, -1])
    }
    colnames(feasible) <- c("index", "n0", "n1", "eS", "eT", "argmax alpha",
                            "max alpha", "argmin power", "min power")
    feasible           <- tibble::as_tibble(feasible)
    feasible[, 1:5]    <- dplyr::mutate_if(feasible[, 1:5], is.double,
                                           as.integer)
    feasible           <- dplyr::arrange(feasible, .data$n0,
                                         dplyr::desc(.data$`min power`))
    eS                 <- feasible$eS[1]
    eT                 <- feasible$eT[1]
    n0                 <- feasible$n0[1]
    n1                 <- feasible$n1[1]
    opchar             <-
      single_double_opchar_one_stage(rbind(rep(feasible$`argmax alpha`[1], 2),
                                           feasible$`argmin power`[1] +
                                             c(0, delta)), n0, n1, eS, eT)
    
  } else {
    eS                 <- eT <- feasible <- n0 <- n1 <- opchar <- NULL
    if (summary) {
      message(uc("two_elip"), "no feasible designs identified in range of ",
              "considered maximal allowed sample size. Consider increasing ",
              "n0max", uc("two_elip"))
    }
  }
  output               <-
    build_des_one_stage_output(alpha, beta, delta, feasible, n0max, opchar,
                               pi0_alt, pi0_null, ratio, summary, "binomial",
                               list(eS = eS, eT = eT, n0 = n0, n1 = n1))
  output
}

single_double_des_two_stage        <- function(alpha, beta, delta, ratio,
                                               pi0_null, pi0_alt, n0max, equal,
                                               w, pi0_ess, efficacy, futility,
                                               summary) {
  params               <- search_parameters(2, "single_double", n0max, ratio)
  feasible             <-
    single_double_des_two_stage_cpp(alpha, beta, delta, params$poss_n0,
                                    params$poss_n1, params$poss_x,
                                    params$poss_y, point_null, pi0_null,
                                    point_alt, pi0_alt, equal, efficacy,
                                    futility, pi0_ess, summary)
  if (feasible[1, 1] > 0) {
    if (summary) {
      message("...feasible designs identified in the range of considered ",
              "maximal allowed sample size. Now identifying the optimal design",
              "...")
    }
    nrow_feasible      <- nrow(feasible)
    if (nrow_feasible == 1) {
      feasible         <- matrix(c(1, feasible[, 1:2],
                                   params$poss_n1[feasible[, 1]],
                                   params$poss_n1[feasible[, 2]],
                                   feasible[, -(1:2)]), 1)
    } else {
      feasible         <- cbind(1:nrow_feasible, feasible[, 1:2],
                                params$poss_n1[feasible[, 1]],
                                params$poss_n1[feasible[, 2]],
                                feasible[, -(1:2)])
    }
    colnames(feasible) <- c("index", "n01", "n02", "n11", "n12", "eS1", "eS2",
                            "eT1", "eT2", "fS1", "fT1", "argmax alpha",
                            "max alpha", "argmin power", "min power", "ESS0",
                            "ESS1")
    feasible           <- tibble::as_tibble(feasible)
    feasible[, 1:11]   <- dplyr::mutate_if(feasible[, 1:11], is.double,
                                           as.integer)
    feasible           <-
      dplyr::mutate(feasible,
                    `argmax ESS(pi,pi)`       = 0,
                    `max ESS(pi,pi)`          = 0,
                    `argmax_pi0 ESS(pi0,pi1)` = 0,
                    `argmax_pi1 ESS(pi0,pi1)` = 0,
                    `max ESS(pi0,pi1)`        = 0,
                    `max(n)`                  =
                      as.integer(rowSums(feasible[, 2:5])))
    for (i in 1:nrow_feasible) {
      index            <- as.numeric(feasible[i, 2] +
                                       params$max_poss_n0*(feasible[i, 4] - 1))
      max_ESS_1d       <-
        single_double_max_ess_1d_two_stage(as.numeric(feasible[i, 2:3]),
                                           as.numeric(feasible[i, 4:5]),
                                           as.numeric(feasible[i, 6]),
                                           as.numeric(feasible[i, 8]),
                                           as.numeric(feasible[i, 10]),
                                           as.numeric(feasible[i, 11]),
                                           params$poss_x[[index]],
                                           params$poss_y[[index]])
      feasible$`argmax ESS(pi,pi)`[i]       <- max_ESS_1d[1]
      feasible$`max ESS(pi,pi)`[i]          <- max_ESS_1d[2]
      max_ESS_2d       <-
        single_double_max_ess_2d_two_stage(as.numeric(feasible[i, 2:3]),
                                           as.numeric(feasible[i, 4:5]),
                                           as.numeric(feasible[i, 6]),
                                           as.numeric(feasible[i, 8]),
                                           as.numeric(feasible[i, 10]),
                                           as.numeric(feasible[i, 11]),
                                           params$poss_x[[index]],
                                           params$poss_y[[index]])
      feasible$`argmax_pi0 ESS(pi0,pi1)`[i] <- max_ESS_2d[1]
      feasible$`argmax_pi1 ESS(pi0,pi1)`[i] <- max_ESS_2d[2]
      feasible$`max ESS(pi0,pi1)`[i]        <- max_ESS_2d[3]
    }
    feasible$o         <- rowSums(matrix(w, nrow_feasible, 5, byrow = T)*
                                    feasible[, c(16, 17, 19, 22, 23)])
    feasible           <- dplyr::arrange(feasible, .data$o,
                                         dplyr::desc(.data$`min power`))
    if (!efficacy) {
      feasible$eS1     <- Inf
      feasible$eT1     <- Inf
    }
    if (!futility) {
      feasible$fS1     <- -Inf
      feasible$fT1     <- -Inf
    }
    eS1                <- feasible$eS1[1]
    eS2                <- feasible$eS2[1]
    eT1                <- feasible$eT1[1]
    eT2                <- feasible$eT2[1]
    fS1                <- feasible$fS1[1]
    fT1                <- feasible$fT1[1]
    n0                 <- c(feasible$n01[1], feasible$n02[1])
    n1                 <- c(feasible$n11[1], feasible$n12[1])
    opchar             <-
      single_double_opchar_two_stage(rbind(rep(feasible$`argmax alpha`[1], 2),
                                           feasible$`argmin power`[1] +
                                             c(0, delta)), n0, n1, eS1, eT1,
                                     fS1, fT1, eS2, eT2, 1:2)
    if (summary) {
      message("...optimal design identified.")
    }
  } else {
    eS1                <- eS2 <- eT1 <- eT2 <- feasible <- fS1 <- fT1 <- n0 <-
                          n1 <- opchar <- NULL
    if (summary) {
      message(uc("two_elip"), "no feasible designs identified in range of ",
              "considered maximal allowed sample size. Consider increasing ",
              "n0max", uc("two_elip"))
    }
  }
  output               <-
    build_des_two_stage_output(alpha, beta, delta, equal, feasible, n0max,
                               opchar, pi0_alt, pi0_ess, pi0_null, ratio,
                               summary, w, "binomial",
                               list(efficacy = efficacy, eS1 = eS1, eS2 = eS2,
                                    eT1 = eT1, eT2 = eT2, fS1 = fS1, fT1 = fT1, 
                                    futility = futility, n0 = n0, n1 = n1))
  output
}

single_double_max_ess_2d_two_stage <- function(n0, n1, eS1, eT1, fS1, fT1,
                                               poss_x1, poss_y1) {
  max_ESS_2d <- stats::optim(par     = c(0.5, 0.5),
                             fn      = single_double_minus_ess_two_stage,
                             n0      = n0,
                             n1      = n1,
                             eS1     = eS1,
                             eT1     = eT1,
                             fS1     = fS1,
                             fT1     = fT1,
                             poss_x1 = poss_x1,
                             poss_y1 = poss_y1)
  c(max_ESS_2d$par, -max_ESS_2d$value)
}

single_double_minus_ess_two_stage  <- function(pi, n0, n1, eS1, eT1, fS1, fT1,
                                               poss_x1, poss_y1) {
  if (!missing(poss_x1)) {
    -single_double_des_ess_two_stage(pi, n0, n1, eS1, eT1, fS1, fT1, poss_x1,
                                     poss_y1)
  } else {
    -single_double_ess_two_stage(pi, n0, n1, eS1, eT1, fS1, fT1)
  }
}

single_double_opchar_one_stage     <- function(pi, n0, n1, eS, eT, pmf_pi) {
  if (missing(pmf_pi)) {
    pmf_pi <- single_double_pmf_one_stage(pi, n0, n1, eS, eT)
  }
  rows_pi  <- nrow(pi)
  P        <- numeric(rows_pi)
  for (i in 1:rows_pi) {
    P[i]   <- sum(dplyr::filter(pmf_pi, .data$pi0 == pi[i, 1] &
                                  .data$pi1 == pi[i, 2] &
                                  .data$decision == "Reject")$`f(x,m|pi)`)
  }
  return(tibble::tibble(pi0           = pi[, 1],
                        pi1           = pi[, 2],
                        `P(pi)`       = P))
}

single_double_opchar_two_stage     <- function(pi, n0, n1, eS1, eT1, fS1, fT1,
                                               eS2, eT2, k, pmf_pi) {
  if (missing(pmf_pi)) {
    pmf_pi         <- single_double_pmf_two_stage(pi, n0, n1, eS1, eT1, fS1,
                                                  fT1, eS2, eT2, k)
  }
  rows_pi          <- nrow(pi)
  n                <- c(n0[1] + n1[1], sum(n0) + sum(n1))
  opchar           <- matrix(0, rows_pi, 15)
  E                <- Fu <- numeric(2)
  for (i in 1:rows_pi) {
    for (j in k) {
      E[j]         <- sum(dplyr::filter(pmf_pi, .data$pi0 == pi[i, 1] &
                                          .data$pi1 == pi[i, 2] &
                                          .data$decision == "Reject" &
                                          .data$k == j)$`f(x,m|pi)`)
      Fu[j]        <- sum(dplyr::filter(pmf_pi, .data$pi0 == pi[i, 1] &
                                          .data$pi1 == pi[i, 2] &
                                          .data$decision == "Do not reject" &
                                          .data$k == j)$`f(x,m|pi)`)
    }
    cum_S          <- cumsum(S <- E + Fu)
    MSS            <- ifelse(any(cum_S == 0.5),
                             0.5*(n[which(cum_S == 0.5)] +
                                    n[which(cum_S == 0.5) + 1]),
                             n[which(cum_S > 0.5)[1]])
    opchar[i, ]    <- c(pi[i, 1], pi[i, 2], sum(E), sum(n*S),
                        sqrt(sum(n^2*S) - sum(n*S)^2), MSS, E, Fu, S, cum_S,
                        n[2])
  }
  opchar           <- tibble::as_tibble(opchar)
  colnames(opchar) <- c("pi0", "pi1", "P(pi)", "ESS(pi)", "SDSS(pi)", "MSS(pi)",
                        paste(rep(c("E", "F", "S"), each = 2), rep(1:2, 3),
                              "(pi)", sep = ""),
                        paste("cum{S", 1:2, "(pi)}", sep = ""), "max(n)")
  opchar$`max(n)`  <- as.integer(opchar$`max(n)`)
  opchar
}

single_double_pmf_one_stage        <- function(pi, n0, n1, eS, eT) {
  x                                        <- expand.grid(0:n0, 0:n1)
  rows_pmf                                 <- (n0 + 1)*(n1 + 1)
  rows_pi                                  <- nrow(pi)
  rows_total                               <- rows_pmf*rows_pi
  f                                        <- numeric(rows_total)
  for (i in 1:rows_pi) {
    dbinom0                                <- stats::dbinom(0:n0, n0, pi[i, 1])
    if (all(n0 == n1, pi[i, 1] == pi[i, 2])) {
      dbinom1                              <- dbinom0
    } else {
      dbinom1                              <- stats::dbinom(0:n1, n1, pi[i, 2])
    }
    f[(1 + (i - 1)*rows_pmf):(i*rows_pmf)] <-
      dbinom0[x[, 1] + 1]*dbinom1[x[, 2] + 1]
  }
  pmf                                      <-
    tibble::tibble(pi0         = rep(pi[, 1], each = rows_pmf),
                   pi1         = rep(pi[, 2], each = rows_pmf),
                   x0          = rep(as.vector(x[, 1]), rows_pi),
                   x1          = rep(as.vector(x[, 2]), rows_pi),
                   m0          = rep(as.integer(n0), rows_total),
                   m1          = rep(as.integer(n1), rows_total),
                   statisticS  = .data$x1,
                   statisticD  = .data$x1 - .data$x0,
                   decision    = ifelse((.data$statisticS >= eS) &
                                          (.data$statisticD >= eT),
                                        "Reject", "Do not reject"),
                   k           = factor(rep(1, rows_total), 1),
                   `f(x,m|pi)` = f)
  dplyr::arrange(pmf, .data$pi0, .data$pi1, .data$x0, .data$x1)
}

single_double_pmf_two_stage        <- function(pi, n0, n1, eS1, eT1, fS1, fT1,
                                               eS2, eT2, k) {
  if (eS1 == Inf) {
    eS1                                          <- n1[1] + 1
  }
  if (eT1 == Inf) {
    eT1                                          <- n1[1] + 1
  }
  if (fS1 == -Inf) {
    fS1                                          <- -1
  }
  if (fT1 == -Inf) {
    fT1                                          <- -n0[1] - 1
  }
  pmf                                            <-
    single_double_pmf_two_stage_cpp(pi[1, ], n0, n1, eS1, eT1, fS1, fT1, eS2,
                                    eT2, k)
  rows_pmf                                       <- nrow(pmf)
  rows_pi                                        <- nrow(pi)
  if (rows_pi > 1) {
    pmf                                          <-
      rbind(pmf, matrix(0, (rows_pi - 1)*rows_pmf, 9))
    for (i in 2:rows_pi) {
      pmf[(1 + (i - 1)*rows_pmf):(i*rows_pmf), ] <-
        single_double_pmf_two_stage_cpp(pi[i, ], n0, n1, eS1, eT1, fS1, fT1,
                                        eS2, eT2, k)
    }
  }
  pmf                                            <-
    tibble::tibble(pi0         = rep(pi[, 1], each = rows_pmf),
                   pi1         = rep(pi[, 2], each = rows_pmf),
                   x0          = as.integer(pmf[, 1]),
                   x1          = as.integer(pmf[, 2]),
                   m0          = as.integer(pmf[, 3]),
                   m1          = as.integer(pmf[, 4]),
                   statisticS  = as.integer(pmf[, 5]),
                   statisticD  = as.integer(pmf[, 6]),
                   decision    = ifelse(pmf[, 7] == 1, "Reject",
                                        "Do not reject"),
                   k           = factor(pmf[, 8], k),
                   `f(x,m|pi)` = pmf[, 9])
   dplyr::arrange(pmf, .data$pi0, .data$pi1, .data$k, .data$x0, .data$x1)
}

single_double_terminal_one_stage   <- function(n0, n1, eS, eT) {
  x        <- expand.grid(0:n0, 0:n1)
  rows_pmf <- (n0 + 1)*(n1 + 1)
  tibble::tibble(x0         = x[, 1],
                 x1         = x[, 2],
                 m0         = rep(as.integer(n0), rows_pmf),
                 m1         = rep(as.integer(n1), rows_pmf),
                 statisticS = .data$x1,
                 statisticT = .data$x1 - .data$x0,
                 decision   = ifelse(all(.data$statisticS >= eS,
                                         .data$statisticT >= eT),
                                     "Reject", "Do not reject"),
                 k          = factor(rep(1, rows_pmf), 1))
}

single_double_terminal_two_stage   <- function(n0, n1, eS1, eT1, fS1, fT1, eS2,
                                               eT2, k) {
  if (eS1 == Inf) {
    eS1    <- n1[1] + 1
  }
  if (eT1 == Inf) {
    eT1    <- n1[1] + 1
  }
  if (fS1 == -Inf) {
    fS1    <- -1
  }
  if (fT1 == -Inf) {
    fT1    <- -n0[1] - 1
  }
  terminal <- single_double_terminal_two_stage_cpp(n0, n1, eS1, eT1, fS1, fT1,
                                                   eS2, eT2, k)
  terminal <- tibble::tibble(x0         = as.integer(terminal[, 1]),
                             x1         = as.integer(terminal[, 2]),
                             m0         = as.integer(terminal[, 3]),
                             m1         = as.integer(terminal[, 4]),
                             statisticS = as.integer(terminal[, 5]),
                             statisticD = as.integer(terminal[, 6]),
                             decision   = ifelse(terminal[, 7] == 1, "Reject",
                                                 "Do not reject"),
                             k          = factor(terminal[, 8], k))
  dplyr::arrange(terminal, .data$k, .data$x0, .data$x1)
}