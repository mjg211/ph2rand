bernard_des_one_stage        <- function(alpha, beta, delta, ratio, pi0_null,
  ...
  feasible             <-
    bernard_des_one_stage_cpp(alpha, beta, delta, params$poss_n0,
                              ...
                              params$unique_B, (length(pi0_null) == 1),
  ...
  if (feasible[1, 1] > 0) {
    ...
    feasible[, 1:3]    <- dplyr::mutate_if(feasible[, 1:3], is.double,
                                           as.integer)
    feasible           <- dplyr::arrange(feasible, n0,
  ...
  output               <-
    build_des_one_stage_output(alpha, beta, delta, feasible, n0max, opchar,
                               pi0_alt, pi0_null, ratio, summary, "bernard",
                               list(e = e, n0 = n0, n1 = n1))
  output
}

bernard_des_two_stage        <- function(alpha, beta, delta, ratio, pi0_null,
...
bernard_terminal_two_stage   <- function(n0, n1, e1, f1, e2, k) {
  ...
  terminal <- bernard_terminal_two_stage_cpp(n0, n1, e1, f1, e2, k)
