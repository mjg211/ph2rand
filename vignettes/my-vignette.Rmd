---
title: "ph2rand: Design of randomized comparative phase II oncology trials with a Bernoulli primary outcome"
author: "Michael J Grayling (michael.grayling@newcastle.ac.uk)"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ph2rand: Design of randomized comparative phase II oncology trials with a Bernoulli primary outcome}
...
Defaults to `0.2`.
- `ratio`: A `numeric` indicating the chosen value for $r$, the allocation ratio to the experimental arm, relative to the control arm. Defaults to \code{1}.
...
Defaults to `T`.
- `efficacy_type`: Only used if `type` is \code{"fisher"}.
...
```{r eg41_3, cache = T, dpi = 150, fig.retina = 4}
...
```{r eg41_4, cache = T, dpi = 150, fig.retina = 4}
...
```{r eg41_6, cache = T, dpi = 150, fig.retina = 4}
plot(des)
```
...
```{r eg43_1, cache = T, dpi = 150, fig.retina = 4}
...
```{r eg43_2, cache = T, dpi = 150, fig.retina = 4}
...
```{r eg44_4, cache = T, dpi = 150, fig.retina = 4}
